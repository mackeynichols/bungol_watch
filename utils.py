...
class BungolScraper:
	...
	def __init__(self, address):
		...
		# headers we'll need to make requests
		self.req_headers = {
			"Host":"www.bungol.ca",
		...
		self.login_url = "/get-listing-data/c3940258-549696/"
	...
	def get_address_coordinates(self):
		...
		response =  json.loads(self.req.get( "https://www.bungol.ca/api/property-search-autocomplete/?value="+self.address.replace(" ", "%20"), data = json.dumps(login_payload), headers = self.req_headers ).text )

		return { "latitude" : response["addresses"][0]['la'], "longitude" : response["addresses"][0]['lo'] }
	...
	def get_address_payload(self):
		...
		response = self.req.get( self.base_api_url + self.login_url, data = json.dumps(login_payload), headers = self.req_headers ) 
		response_payload = [ result for result in json.loads(response.text)["results"] if result['street'].upper() ==  self.address.upper() ]
